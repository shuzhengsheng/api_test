...
# 获取工作表的数据（字典）
def get_from_sheetname(sheet_name):
    request_list = []
    url_path = os.path.join(proDir, 'testFile', 'interfaceURL.xml')
    tree = ElementTree.parse(url_path)
    for u in tree.findall('url'):
        url_name = u.get('name')
        if url_name == sheet_name:

            rows = get_xls('userCase.xlsx', u.find('action').text)
            for row in rows:
                case_data = dict()
                case_data['case_name'] = row[0].strip()
                case_data['method'] = row[1].strip().upper()
                case_data['url'] = row[2].strip()
                if row[3] is not None and len(row[3].strip()) > 0:
                    case_data['data'] = row[3].replace("\n", "")
                    if row[3].strip().startswith('['):
                        case_data['type'] = 'Array'
                        tmp_data = '{"key":' + row[3].replace("\n", "") + '}'
                        case_data['data'] = json.loads(tmp_data)["key"]
                    elif row[3].strip().startswith('{'):
                        case_data['type'] = 'Object'
                        case_data['data'] = json.loads(row[3].replace("\n", ""))
                    else:
                        print('data数据类型有误')
                else:
                    case_data['data'] = {}
                request_list.append(case_data)
    return request_list


if __name__ == "__main__":
    print(get_xls("userCase.xlsx", "unit_save"))
